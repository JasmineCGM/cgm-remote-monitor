...
var alarms = {
    ...
    "LowLowAlarm": new Alarm("Low_Low_Alarm", 60),
    "LowAlarm": new Alarm("Low_Alarm", 90),
    "LowFallingAlarm": new Alarm("Low_Falling_Alarm", 180),
    "HighAlarmDay": new Alarm("High_Alarm_Day", 230),
    "HighAlarmNight": new Alarm("High_Alarm_Night", 260),
    "HighRiseAlarm": new Alarm("High_Rise_Alarm", 300)
};
...
function loadData() {
    ...
    if (actualLength > 1) {
        ...
        var currentBG = actual[actualLength].y;
        var hourNow = new Date().getHours();
        var currentDirection = actual[actualLength].direction;
        var minLastReading = (now - lastValidReadingTime)/60/1000;
        //console.log("now: " + now);
        //console.log("lastValidReadingTime: " + lastValidReadingTime);
        //console.log("minLastReading: " + minLastReading);
        
        
        if(errorCode == 'false' && minLastReading > 21){
            emitAlarm('alarm'); 
        }else if (currentBG < alarms['LowLowAlarm'].threshold){
            emitAlarm('LowLowAlarm');
        }else if(currentBG < alarms['LowAlarm'].threshold){
            emitAlarm('LowAlarm');
        }else if(currentBG < alarms['LowFallingAlarm'].threshold && currentDirection == '&#8650;'){
            emitAlarm('LowFallingAlarm');
        }else if(currentBG > alarms['HighRiseAlarm'].threshold && currentDirection == '&#8648;'){
            emitAlarm('HighRiseAlarm');
        //hour run on server and are 6 hours ahead for 8am to 8pm is 14 to 2
        }else if(currentBG > alarms['HighAlarmDay'].threshold && hourNow > 14){
            emitAlarm('HighAlarmDay');
        }else if(currentBG > alarms['HighAlarmDay'].threshold && hourNow < 2){
            emitAlarm('HighAlarmDay');
        }else if(currentBG > alarms['HighAlarmNight'].threshold){
            emitAlarm('HighAlarmNight');
        }else if (errorCode) {
            emitAlarm('urgent_alarm');
        }else if (minLastReading > 21){
            emitAlarm('alarm');   
        }    
    }
}
